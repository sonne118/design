...
var urls = new[] { "http://www.google.com", "http://www.excella.com", "http://www.bing.com" };
var downloads = urls.Select(DownloadPage).ToList();

while (downloads.Count > 0) 
{
	var tempResult = await Task.WhenAny(downloads);   
	downloads.Remove(tempResult);   
	var result = await tempResult;

	Console.WriteLine("{0} (length {1})", result.Item1, result.Item2);
}
